...
class Smooth_Select(Reduction):
    ...
    def get_pick(self):
        ...
        if istype(self.reduction_data, dict):
            picked = {}
            for channel, fft_trace in self.reduction_data.items:
                if self.smoothing.lower() == 'konno_ohmachi' and channel != 'freqs':
